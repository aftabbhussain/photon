...
// Health check endpoint for browsers and Render proxy
app.get('/', (_req, res) => {
  res.status(200).json({ status: 'healthy', service: 'signaling-server' });
});
...
wss.on('connection', (ws: WebSocket) => {
      ...
      switch (parsed.type) {
        ...
        case 'join-room': {
          ...
          if (result.success) {
            send(ws, {
              type: 'room-joined',
              payload: { roomCode: roomCode.toUpperCase(), peerId: result.peerId }
            });

            if (result.hostSocket) {
              send(result.hostSocket, {
                type: 'peer-joined',
        ...
        case 'ice-candidate': {
          ...
          if (!meta) {
            send(ws, {
              type: 'error',
              payload: { message: 'No socket to peer mapping found, join a room first' }
            });
            return;
          }
        ...
        default:
          send(ws, {
            type: 'error',
            payload: { message: `Action ${parsed.type} is unsupported at this stage` }
          });
      }
    } catch (err) {
  ...
  ws.on('close', () => {
    ...
    if (closedDetails && closedDetails.remainingPeerSocket) {
      send(closedDetails.remainingPeerSocket, {
        type: 'peer-left',
        payload: { peerId: closedDetails.peerId }
      });
    }
...
server.listen(PORT, () => {
  console.log(`Signaling server initialized on port ${PORT}`);
});
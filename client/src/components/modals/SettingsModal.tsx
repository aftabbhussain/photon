...
export const SettingsModal:React.FC<SettingsModalProps>=({
  ...
  return(
    ...
    >
      <div
        className="bg-white border border-slate-700/60 rounded-2xl md:rounded-3xl w-auto max-w-[460px]c p-5 relative shadow-2xl"
        onClick={(e)=>e.stopPropagation()}
      >
        <button
          className="absolute top-4 right-4 w-8 h-8 rounded-full bg-[#1e2434] flex items-center justify-center text-white hover:bg-red-700 hover:text-white transition"
          onClick={onClose}
        >
          <X size={16}/>
        </button>
        ...
        <p className="text-xs md:text-sm text-slate-500 mb-5 leading-relaxed">
          Signaling Server &amp; Data Channel Health
        </p>

        <div className="bg-black w-auto p-4 rounded-xl mb-3 text-xs sm:text-sm flex flex-col gap-2.5 border border-[#222938]">
